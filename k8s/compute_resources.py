...
def compute_allocations(total_cpu: int, total_ram_mib: int) -> dict[str, Resources]:
    ...
    print(f"✅  Usable after headroom: {usable_cpu_m}m CPU, {usable_ram_mib} MiB RAM")
    ...
    return allocs, usable_cpu_m, usable_ram_mib
...
def write_values_computed(
    ...
    lines = [
        ...
        f"# Usable after headroom: {usable_cpu_m}m CPU, {usable_ram_mib} MiB RAM",
        "",
        "# ── backend app ──────────────────────────────────────────────────────────────",
        "resources:",
...
def write_minikube_env(
    ...
    # Cap memory to what Docker's VM actually has (OrbStack / Docker Desktop
    # expose less RAM than the host machine). Use 90% to leave headroom.
    docker_mem = docker_available_memory_mib()
    if docker_mem is not None:
        docker_cap = int(docker_mem * 0.90)
        if usable_ram_mib > docker_cap:
            print(f"⚠️  Docker VM has {docker_mem} MiB — capping minikube memory "
                  f"from {usable_ram_mib} MiB → {docker_cap} MiB")
            usable_ram_mib = docker_cap
