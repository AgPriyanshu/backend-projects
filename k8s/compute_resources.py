...
def detect_gpu_vram_mib() -> int:
    """Returns VRAM in MiB of the first NVIDIA GPU, or 0 if none found."""
    try:
...
class Resources:
    ...
    def to_yaml_block(self, indent: int = 0, gpu_count: int = 0) -> str:
        pad = " " * indent
        gpu = f'{pad}  "nvidia.com/gpu": "{gpu_count}"\n' if gpu_count else ""
        return (
            f"{pad}requests:\n"
            f"{gpu}"
...
def write_values_computed(
    ...
    gpu_line = f", GPU {gpu_vram_mib} MiB VRAM" if has_gpu else ""
    lines = [
        "# Auto-generated by compute_resources.py — DO NOT EDIT MANUALLY",
        f"# Machine: {total_cpu} cores, {total_ram_mib} MiB RAM{gpu_line} ({platform.system()})",
        ...
        "# ── vllm inference ────────────────────────────────────────────────────────────",
        "vllmResources:",
        allocs["vllm"].to_yaml_block(indent=2, gpu_count=1),
        "",
        f'vllmPersistence:\n  size: "{storage["vllm"]}Gi"',
    ]
...
def run(script_dir: Path = SCRIPT_DIR) -> dict:
    ...
    has_gpu = detect_gpu()
    gpu_vram_mib = detect_gpu_vram_mib() if has_gpu else 0

    if has_gpu:
        print(f"🎮 GPU detected ({gpu_vram_mib} MiB VRAM) — minikube will start with GPU passthrough")
    else:
